...
import es.caib.zkib.jxpath.ri.compiler.Step;
import es.caib.zkib.jxpath.ri.model.NodeIterator;
import es.caib.zkib.jxpath.ri.model.NodePointer;
import es.caib.zkib.jxpath.ri.model.beans.LangAttributePointer;
import es.caib.zkib.jxpath.ri.model.beans.NullElementPointer;
import es.caib.zkib.jxpath.ri.model.beans.NullPropertyPointer;
import es.caib.zkib.jxpath.ri.model.beans.PropertyOwnerPointer;
import es.caib.zkib.jxpath.ri.model.beans.PropertyPointer;
...
//        return valuePointer(pointer);
            ...
            if (it != null) {
                if (predicate instanceof NameAttributeTest) { // [@name = key]
                    ...
                    for (int i = 1; it.setPosition(i); i++) {
                        ...
                        if (isNameAttributeEqual(ptr, key, context.getJXPathContext().isIgnoreExceptions())) {
                            pointer = ptr;
                            break;
                        }
                    }
    ...
    private static NodePointer doPredicateName(
        ...
        if (child instanceof PropertyOwnerPointer) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                else if (isNameAttributeEqual(valuePointer, key, context.getJXPathContext().isIgnoreExceptions())) {
                    pointer =
                        doPredicate(
                            context,
                            valuePointer,
    ...
    private static NodePointer doPredicatesStandard(
        ...
        if (predicate instanceof NameAttributeTest) {
            ...
            for (int i = 0; i < parents.size(); i++) {
                ...
                if (isNameAttributeEqual(pointer, key, context.getJXPathContext().isIgnoreExceptions())) {
                    newList.add(pointer);
                }
            }
            if (newList.size() == 0) {
    ...
    private static boolean isNameAttributeEqual(
        NodePointer pointer,
        String name, boolean ignoreExceptions) {
        NodeIterator it = pointer.attributeIterator(QNAME_NAME, ignoreExceptions);
        return it != null
            && it.setPosition(1)
    ...
    private static NodeIterator getNodeIterator(
        ...
        if (step.getAxis() == Compiler.AXIS_CHILD) {
            ...
            return pointer.childIterator(nodeTest, false, null, context.getJXPathContext().isIgnoreExceptions());
        }
        ...
        return pointer.attributeIterator(
            ((NodeNameTest) step.getNodeTest()).getNodeName(), false);
    }
