...
public abstract class PropertyOwnerPointer extends NodePointer {
    ...
    public NodeIterator childIterator(NodeTest test, boolean reverse,
            NodePointer startWith, boolean ignoreExceptions) {
        if (test == null) {
            return createNodeIterator(null, reverse, startWith, ignoreExceptions);
        }
        if (test instanceof NodeNameTest) {
            ...
            if (isValidProperty(testName)) {
                return createNodeIterator(nodeNameTest.isWildcard() ? null
                        : testName.toString(), reverse, startWith, ignoreExceptions);
            }
            return null;
        }
        return test instanceof NodeTypeTest && ((NodeTypeTest) test).getNodeType() == Compiler.NODE_TYPE_NODE
                ? createNodeIterator(null, reverse, startWith, ignoreExceptions) : null;
    }
    ...
    public NodeIterator attributeIterator(QName name, boolean ignoreExceptions) {
        return new BeanAttributeIterator(this, name, ignoreExceptions);
    }
